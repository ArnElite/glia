...
Correctness is measured per-query by scoring term-overlap between the
response and the source document, then compared across tracks in the report.
"""
...
# ── NEW: correctness checker ──────────────────────────────────────────────────
from runner.correctness_checker import compare_tracks

log = logging.getLogger("glia.benchmark")
...
def _simulated_llm_call(prompt: str, context: str) -> Tuple[str, int]:
    """Simulate an LLM call (50–250ms). Returns (response_text, tokens_used)."""
...
class BenchmarkPacket:
    ...
    query_text:        str
    response_text:     str             # actual response stored here
    latency_ms:        float
    cache_result:      Optional[str]   # "hit" | "miss" | None
    tokens_used:       int
    source_id:         Optional[str]
    correctness_score: float = 0.0    # 0.0–1.0 term-overlap score
    is_stale:          bool  = False


# ─────────────────────────────────────────────────────────────────────────────
# Per-DB retrieval stubs
# ─────────────────────────────────────────────────────────────────────────────
    ...
    def aggregate(self) -> Dict:
        ...
        for track in ("glia", "standard"):
            ...
            for db in ("relational", "graph", "vector"):
                ...
                correctness  = [p.correctness_score for p in sub]
                ...
                result[track][db] = {
                    ...
                    "query_count":   len(sub),
                    "correctness":   stats(correctness),   # ← NEW
                }

        # Correctness comparison summary across all queries
        std_all  = [p.correctness_score for p in packets if p.track == "standard"]
        glia_all = [p.correctness_score for p in packets if p.track == "glia"]
        result["correctness_comparison"] = {
            "standard_mean": round(sum(std_all)  / max(len(std_all),  1), 3),
            "glia_mean":     round(sum(glia_all) / max(len(glia_all), 1), 3),
            "glia_wins":     sum(1 for s, g in zip(std_all, glia_all) if g > s + 0.05),
            "std_wins":      sum(1 for s, g in zip(std_all, glia_all) if s > g + 0.05),
            "ties":          sum(1 for s, g in zip(std_all, glia_all) if abs(s - g) <= 0.05),
        }
...
class BenchmarkRunner:
    ...
    def _on_cache_hit(self, event) -> None:
        self._publish("cache_hit", {"source_id": event.source_id, "payload": event.payload})
    ...
    def _on_invalidation(self, event) -> None:
        ...
        self._publish("invalidation", {
            "source_id": source_id, "deleted_count": event.deleted_count, "latency_ms": lat_ms,
        })
    ...
    def _start_watcher(self) -> None:
        ...
        if adapters:
            self._watcher = CacheWatcher(
                invalidator=invalidator, adapters=adapters, emitter=self._emitter,
            )
            self._watcher.start()
    ...
    def _run_query_standard(self, doc: Dict, db_type: str) -> BenchmarkPacket:
        ...
        return BenchmarkPacket(
            ...
            latency_ms=latency_ms, cache_result=None, tokens_used=tokens,
    ...
    def _run_query_glia(self, doc: Dict, db_type: str) -> BenchmarkPacket:
        ...
        if cached is not None:
            ...
            return BenchmarkPacket(
                ...
                query_text=query, response_text=cached,
                latency_ms=latency_ms, cache_result="hit", tokens_used=0,
                source_id=source_id,
            )
        ...
        return BenchmarkPacket(
            timestamp=time.time(), track="glia", db_type=db_type,
            query_text=query, response_text=response,
            latency_ms=latency_ms, cache_result="miss", tokens_used=tokens,
            source_id=source_id,
        )

    def _run_query_pair(self, doc: Dict, db_type: str):
        """
        Run both tracks for the same doc, then score correctness for each.
        Returns (pkt_standard, pkt_glia).
        """
        pkt_std  = self._run_query_standard(doc, db_type)
        pkt_glia = self._run_query_glia(doc, db_type)

        comparison = compare_tracks(
            standard_response=pkt_std.response_text,
            glia_response=pkt_glia.response_text,
            source_doc=doc,
        )

        pkt_std.correctness_score  = comparison["standard_score"]
        pkt_glia.correctness_score = comparison["glia_score"]

        log.debug(
            "Correctness | doc=%s db=%s | std=%.2f glia=%.2f winner=%s terms=%s",
            doc.get("id", "?"), db_type,
            comparison["standard_score"], comparison["glia_score"],
            comparison["winner"], comparison["key_terms"][:4],
        )

        return pkt_std, pkt_glia
    ...
    def run(self) -> Dict:
        ...
        self._simulator = VolatilitySimulator(
            all_doc_ids=doc_ids,
            on_cycle=lambda c, n: self._publish("simulator_cycle", {"cycle": c, "mutations": n}),
        )
        ...
        for doc in sample:
            for db_type in db_types:
                # ── Both tracks run together so correctness is compared ──────
                pkt_std, pkt_glia = self._run_query_pair(doc, db_type)
            ...
            if completed % 100 == 0:
                ...
                self._publish("progress", {"completed": completed, "total": len(sample), "aggregate": agg})
