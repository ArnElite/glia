...
def _print_report(report: dict) -> None:
    ...
    # ── Correctness table (NEW) ───────────────────────────────────────────────
    print(f"\n  {'CORRECTNESS (0–1 term overlap)':30s}  {'Standard':>12}  {'Glia':>12}  {'Δ':>10}")
    print(f"  {'─'*66}")
    for db in db_types:
        std_c  = res.get("standard",{}).get(db,{}).get("correctness",{}).get("mean",0)
        glia_c = res.get("glia",   {}).get(db,{}).get("correctness",{}).get("mean",0)
        delta  = glia_c - std_c
        sign   = "↑" if delta > 0.001 else ("↓" if delta < -0.001 else "=")
        print(f"  {db.upper()+' mean':30s}  {std_c:>12.3f}  {glia_c:>12.3f}  "
              f"{sign}{abs(delta):>8.3f}")

    # ── Correctness win/loss summary ──────────────────────────────────────────
    cc = res.get("correctness_comparison", {})
    if cc:
        total_comparisons = cc.get("glia_wins", 0) + cc.get("std_wins", 0) + cc.get("ties", 0)
        print(f"\n  Correctness head-to-head ({total_comparisons} query pairs):")
        print(f"    Glia wins:     {cc.get('glia_wins', 0):>5}")
        print(f"    Standard wins: {cc.get('std_wins',  0):>5}")
        print(f"    Ties (±0.05):  {cc.get('ties',      0):>5}")
        print(f"    Overall — Standard: {cc.get('standard_mean',0):.3f}  "
              f"Glia: {cc.get('glia_mean',0):.3f}")
...
def main() -> None:
    ...
    if not args.skip_seed:
        ...
        from config.settings import SEED_RECORD_COUNT, DOCUMENT_CATEGORIES
        import random, uuid
